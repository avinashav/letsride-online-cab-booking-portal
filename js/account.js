...
var AccountWrapper = function () {
    ...
    var properties = {
        ...
        'Error_400': 'Error. Unqualified request',
        'Error_401': 'Error. User not authorized',
        'Error_404': 'Error. Specified resource not found',

        'bookingAcceptedMessage': 'Operation completed successfully',
        'bookingAcceptingErrorMessage': 'An error occurred during the operation',
    ...
    var BOOKING_STATE_DICTIONARY = {
        'AWAITING_TAXI': 'Waiting for a taxi',
        'CANCELED_BOOKING': 'Order cancelled',
        'COMPLETED_BOOKING': 'Order Complete',
        'PASSENGER_PICKED_UP': 'Landing completed',
        'TAXI_DISPATCHED': 'Taxi left'
    }
    ...
    function findActiveBookingDone(data) {
        if (data.status === "0") {
            $('.options__current_order>p.order_info').html("Of " + data.data.route.start_address.address + " at " + data.data.route.end_address.address + ". " + data.data.cost + " rub.");
    ...
    function cancelCurrentOrderDone(data) {
        if (data.status === "0") {
            $('.options__current_order>p.order_info').html("No orders found");
        ...
        for (var i = 0; i < orders.length; i++) {
           if(flag===1){
            ...
            $('#history_table > tbody:last-child').append("<tr><td>" + BOOKING_STATE_DICTIONARY[orders[i].state] + "</td><td>"
                ...
                + orders[i].route.distance / properties.kilometer + "km." + "</td><td>"
    ...
    function generatePickUpButtonIntoTable(id){
        return "<td><button class = 'pickup' id=pickup" + id + ">Passenger boarding</button></td>";
    }
    ...
    function generateDropOffButtonIntoTable(id){
        return "<td><button class='dropoff' id=dropoff" + id + ">Leaving of the passenger</button></td>";
    }
    ...
    function generateReportDone(data){
        var docDefinition = {
            header: function (currentPage, pageCount) {
                return {
                    text: 'Report of taxi orders for the user ' + JSON.parse(Cookies.get('user-info')).family_name + " " + JSON.parse(Cookies.get('user-info')).common_name,
                    style: 'header'
                };
            },
    ...
    function createReportData(data){
        var reportData = [
            [ 'Status', 'Date', 'Point of departure', 'Destination point', 'Dist.', 'Cost' ],
        ];
        var orders = data.data;
        for(var i = 0 ; i < orders.length; i++) {
    ...
    function findAwaitingTaxiDone(data){
        ...
        for(var i = 0 ; i < orders.length; i++) {
            $('#awaiting_taxi_table > tbody:last-child').append("<tr><td>" + BOOKING_STATE_DICTIONARY[orders[i].state] + "</td><td>"
                ...
                + "<td><button class='acceptTaxi' id="+ orders[i].id + ">Accept order</button></td>" +"</tr>"
            );
        }
