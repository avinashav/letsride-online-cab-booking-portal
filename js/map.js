...
function placeMarkerAndPanTo(latLng, map) {
    ...
    if (markers.length >= 2) {
        $('.alert.warning').html('<span class="closebtn" onclick="toggle()">&times;</span><strong>Oops!</strong> The path can not contain more than 2 points').toggle();
    ...
    geocoder.geocode({'location': marker.position}, function (results, status) {
        ...
        if (status === 'OK') {
            if (results[0]) {
                ...
                $('.alert.warning').html('<span class="closebtn"  onclick="toggle()">&times;</span><strong>Oops!</strong> Location not found').show();
            }
        } else {
            //TO-DO Create beautiful notification
            $('.alert.danger').html('<span class="closebtn"  onclick="toggle()">&times;</span><strong>Oops!</strong> Geocoder error: ' + status).show();
        }
    });
}
...
function showRouteInfo(totalDistance, totalDuration, totalCost) {
    ...
    $('#totalDuration').find('span').text(Math.round(totalDuration) + " mines.");
